...
skyGradient.addColorStop(0,"#90dffe");
skyGradient.addColorStop(1,"#38a3d1");
  
  const earthquakeGradient = context.createLinearGradient(0, 0, 0, canvas.height)
  earthquakeGradient.addColorStop(0, "#163C52")
  earthquakeGradient.addColorStop(0.25, "#4F4F47")
  earthquakeGradient.addColorStop(0.5, "#C5752D")
  earthquakeGradient.addColorStop(0.75, "#B7490F")
  earthquakeGradient.addColorStop(1, "#2F1107")
  ...
  function drawPlayerAt({
    ...
    if (umbrella) {
      ...
      setText(uname, x + 15, y - 45, {
        color: "black",
        size: 15,
        style: username === uname ? "bold" : "normal"
      });
    setText(`☆${score} ♡${health}`, x + 15, y - 30, {
      color: "black",
      size: 10
    });
  }
  ...
  setInterval(() => {
    ...
    colourRect(0, 0, canvas.width, canvas.height, gameData.earthquake ? earthquakeGradient : skyGradient)
    setText("v0.3.0 The slightly harder update", 20, canvas.height - 20, {
      align: "left",
      size: 20
    });
    ...
    platforms.forEach(({ x, y }, i) => {
      ...
      if (!firstUpdate) platforms[i].y += gameData.earthquake ? 10 : 5;
      if (
        y > player.y - (player.umbrella ? 40 : 0) &&
        player.x > x &&
        player.x < x + 50
      ) {
        if (!justDied || player.umbrella) {
          delete platforms[i];
          _.throttle(() => socket.emit("remove platform", i), 1000)
        }
        if (!justDied && !player.umbrella) {
          player.health -= chance.integer({ min: 1, max: 5 });
          if (player.health <= 0) {
            justDied = true;
            player.health = 0;
            setTimeout(() => {
    ...
    if (justDied) {
      setText("You Died.", canvas.width / 2, canvas.height / 3);
    }

    setText(player.score, canvas.width / 2, 40, { align: "right" });
    if (paused) {
      colourRect(0, 0, canvas.width, canvas.height, "rgba(0, 0, 0, 0.6)");
      setText("Game paused", canvas.width / 2, canvas.height / 2);
    }
  }, 1000 / 60);
  ...
  window.addEventListener("keypress", ({ key }) => {
    ...
    } else if (key === "g" && player.score >= cost.gapple && player.health < limit.gappledHealth) {
      player.health += _.round((limit.gappledHealth - player.health) /2)
      player.score -= cost.gapple
    }
  });
